...
class Semesters
{
    ...
    /**
     * @return array<int,array{'spring':array{int,int},'summer':array{int,int},'fall':array{int,int}}>
     */
    public static function allDates(): array
    {
        return Cache::get(
            'unm/semester_dates',
            function () {
                $query = SharedDB::query()->from('semester_dates')
                    ->order('year desc, month desc, day desc');
                /** @var array<int,array{'spring':array{int,int},'summer':array{int,int},'fall':array{int,int}}> $dates */
                $dates = [];
                while ($row = $query->fetch()) {
                    $dates[$row['year']][$row['semester']] = [$row['month'], $row['day']];
                }
                return $dates;
            },
            3600 * 8
    ...
    {
        $year = intval($year);
        return static::allDates()[$year]['spring'] ?? static::SPRING_DEFAULT;
    }
    ...
    {
        $year = intval($year);
        return static::allDates()[$year]['summer'] ?? static::SUMMER_DEFAULT;
    }
    ...
    {
        $year = intval($year);
        return static::allDates()[$year]['fall'] ?? static::FALL_DEFAULT;
    }
